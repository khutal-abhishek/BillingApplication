          ...
          <div className='Navbar'>
            <nav className="navbar bg-body-tertiary">
              <div className="container-fluid">
                ...
                <button className="btn btn-success" type="submit" onClick={() => { setEditCategory(null); setModalOpen(true); }}>
                  <span className='plus'><FontAwesomeIcon icon={faPlus} /></span> Add Category
                </button>
              </div>
            </nav>
          </div>
              ...
              <tbody>
                {currentCategories.map((category, index) => (
                  <tr key={index}>
                    ...
                    <td className='catImg'><img src={category.category_image} alt={category.categoryname} width="50" height="50" /></td>
                    <td>{category.categoryname}</td>
                    <td>{category.createdon}</td>
                    <td>
                      <button className='icondel border-0' onClick={() => { setEditCategory(category); setModalOpen(true); }}>
                        <FontAwesomeIcon icon={faEdit} style={{ marginRight: '10px', color: 'green', border: '2px solid green', padding: '3px' }} />
                      </button>
                      <button className='idonupd border-0' onClick={() => deleteCategory(category.categoryid)}>

...
const Category = ({ addItemToBilling }) => {
  ...
  useEffect(() => {
    axios.get("http://localhost:8080/GoMart/product")
      .then((response) => {
  ...
  useEffect(() => {
    ...
    if (selectedCategory) {
      filtered = items.filter(item => item.categoryid === Number(selectedCategory));
    }
            ...
            {filteredItems.length > 0 ? (
              filteredItems.map((item) => (
                <div key={item.productid} className="item-card" onClick={() => addItemToBilling(item)}>
                  <img src={item.product_image} alt={item.productname} className="productImage" />
                  <div className="item-details">
                    <p><span className="fw-bold">{item.productname}</span>
                    <br/>
                    <span>₹ {item.product_price.toFixed(2)}</span></p>
                  </div>
                </div>
              ))
            ) : (

...
export default function ViewProduct() {
  ...
  const loadProducts = async () => {
    try {
      ...
      console.log('Products fetched:', result.data); // Log the fetched products
  ...
  const currentEntries = filteredProducts.slice(offset, offset + entriesPerPage);
          ...
          <div className='Navbar'>
            <nav className="navbar bg-body-tertiary">
              <div className="container-fluid">
                ...
                <button className="btn btn-success" type="button" onClick={() => { setEditProduct(null); setModalOpen(true); }}>
                  <span className='plus'><FontAwesomeIcon icon={faPlus} /></span> Add Product
                </button>
              </div>
            </nav>
          </div>
                  ...
                  currentEntries.map((product, index) => {
                    ...
                    return (
                      <tr key={product.productid || index}>
                        ...
                        <td className='prodImg text-center'><img src={product.product_image} alt={product.productname} width="50" height="50" /></td>
                        <td>{product.productname}</td>
                        <td>{category ? category.categoryname : 'Unknown'}</td>
                        <td>{product.product_price}</td>
                        <td>{product.createdon}</td>
                        <td>
                          <button className='icondel border-0' onClick={() => { setEditProduct(product); setModalOpen(true); }}>
                            <FontAwesomeIcon icon={faEdit} style={{ marginRight: '10px', color: 'green', border: '2px solid green', padding: '3px' }} />
                          </button>
                          <button className='idonupd border-0' onClick={() => deleteProduct(product.productid)}>
                            <FontAwesomeIcon icon={faTrash} style={{ color: 'red', border: '2px solid red', padding: '3px' }} />
                          </button>
                        </td>
                      </tr>
                    )
